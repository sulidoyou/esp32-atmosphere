...
// 节奏序列编码：
// 0=停顿, 1=右电磁铁, 2=左电磁铁, 3=左右同时
// 说明：单击/双击模式会配合 fire_limit 自动停止
static const uint8_t PATTERN_SINGLE[] = {2, 0, 0, 0};              // 单击：1下（左）
static const uint8_t PATTERN_DOUBLE[] = {2, 1, 0, 0};              // 双击：2下（左→右）
static const uint8_t PATTERN_ROLL[] = {2, 1, 2, 1, 2, 1, 2, 1};    // 滚奏
static const uint8_t PATTERN_WALTZ[] = {2, 0, 1, 0, 2, 0, 1, 0};   // 华尔兹
static const uint8_t PATTERN_ROCK[] = {2, 2, 1, 2, 0, 2, 1, 0};    // 摇滚（持续循环）
...
static void beat_timer_callback(TimerHandle_t t)
{
    ...
    // beat值: 0=停顿, 1=右电磁铁, 2=左电磁铁, 3=双面同时
    if (beat == 3) {
        // 拍掌：两边同时敲，分两次队列发送
